  ...
  try {
      ...
      const { data: newUser, error: insertError } = await supabase
          ...
          .insert({
              ...
              password: passwordHash, // Utiliser 'password' au lieu de 'password_hash'
...
export async function loginUser(loginData: UserLoginData, cookies: any): Promise<{ success: boolean; error?: string; user?: User }> {
  try {
      ...
      const { data: user, error: userError } = await supabase
          .from('users')
          .select('id, username, email, password') // Utiliser 'password' au lieu de 'password_hash'
...
export async function logoutUser(token: string, cookies: any): Promise<{ success: boolean; error?: string }> {
  try {
      console.log("Déconnexion de l'utilisateur avec token:", token.substring(0, 10) + "...");

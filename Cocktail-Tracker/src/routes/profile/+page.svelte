<script lang="ts">
    ...
    function handlePasswordSubmit() {
        ...
        return ({ update }: { update: () => void }) => {
            ...
            // Les messages seront affichés via l'objet form retourné par l'action
            if (form?.passwordSuccess) {
                // Si changement réussi, on peut réinitialiser les champs
        ...
        {#if activeTab === "myRecipes"}
            {#if userCocktails.length === 0}
                <div class="empty-state">
                    <p>
                        Vous n'avez pas encore partagé de recettes de cocktail.
                    </p>
                    <a href="/create" class="create-btn"
                        >Créer votre premier cocktail</a
                    >
                </div>
            {:else}
                <div class="cocktails-grid">
                    {#each userCocktails as cocktail (cocktail.id)}
                        <CocktailCard {cocktail} {user} />
                    {/each}
                </div>
            {/if}
            ...
            {:else}
                <div class="cocktails-grid">
                    {#each likedCocktails as cocktail (cocktail.id)}
                        <CocktailCard {cocktail} userVote="like" {user} />
                    {/each}
                </div>
            {/if}
            ...
            {:else}
                <div class="cocktails-grid">
                    {#each dislikedCocktails as cocktail (cocktail.id)}
                        <CocktailCard {cocktail} userVote="dislike" {user} />
                    {/each}
                </div>
            {/if}
            ...
            <div class="settings-panel">
                ...
                <div class="settings-section">
                    ...
                    {#if changePasswordMode}
                        ...
                        >
                            ...
                            <div class="form-group">
                                ...
                                />
                                <small
                                    >Le mot de passe doit comporter au moins 6
                                    caractères</small
                                >
                            </div>
                            ...
                            <!-- Messages d'erreur de validation côté client -->
                            {#if passwordError}
                                <div class="message error">
                                    <p>{passwordError}</p>
                                </div>
                    ...
                    {:else}
                        <button
                            class="password-btn"
                            on:click={() => (changePasswordMode = true)}
                        >
                            Changer le mot de passe
                        </button>
                    {/if}
